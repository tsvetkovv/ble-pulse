(function() {
  ...
  class HeartRateSensor {
      ...
      return window.navigator.bluetooth.getDevices()
        .then(devices => {
          if (devices.length === 0 || reset) {
            return window.navigator.bluetooth.requestDevice({filters:[{services:[ 'heart_rate' ]}]})
          }
          return devices[0];
        })
    }
     ...
     *
     * @param value
     * @return {{heartRate: number, contactDetected: boolean, energyExpended: number, rrIntervals: number[]}}
     */
